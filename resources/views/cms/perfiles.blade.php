    ...
    <div class="perfiles-container">
        ...
        <div class="section-card">
            <div class="section-header">
                ...
                <p class="section-description">Gestiona los perfiles del sistema</p>
            </div>
            <div class="section-content">
                <!-- Controles de la tabla -->
                <div class="datatable-controls">
                    <div class="datatable-search">
                        <input type="text" class="form-control" placeholder="Buscar perfiles..." id="searchPerfiles">
                    </div>
                    <div class="datatable-actions">
                        <button class="btn primary sm" id="refreshPerfiles">
                            <i class="fas fa-sync-alt"></i> Actualizar
                        </button>
                        <button class="btn success sm" id="addPerfil">
                ...
                <div class="empty-state" id="emptyState" style="display: none;">
                    ...
                    <p>No se han encontrado perfiles en el sistema.</p>
                    <button class="btn primary mt-3" id="addFirstPerfil">
                        <i class="fas fa-plus"></i> Crear Primer Perfil
                    </button>
                </div>

                <!-- Loading -->
                <div class="loading" id="loadingPerfiles">
...
<div class="modal-overlay" id="perfilModal">
    <div class="modal">
        <div class="modal-header">
            <h3 class="modal-title" id="modalTitle">Nuevo Perfil</h3>
            <button class="modal-close" data-modal="perfilModal">
        ...
        <div class="modal-footer">
            <button class="btn light" data-modal="perfilModal">Cancelar</button>
            <button class="btn primary" id="savePerfil">
        ...
        <div class="modal-body">
            <div class="alert danger">
                ...
                <div class="alert-content">
                    ...
                    <p class="alert-message" id="confirmMessage">¿Estás seguro de que quieres eliminar este perfil?</p>
                </div>
            </div>
            <p>Perfil a eliminar: <strong id="perfilToDelete"></strong></p>
        </div>
        <div class="modal-footer">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const loadingPerfiles = document.getElementById('loadingPerfiles');
        const searchInput = document.getElementById('searchPerfiles');
        ...
        // ===== RENDERIZAR PERFILES =====
        function renderPerfiles(perfilesList) {
            ...
            const html = perfilesList.map(perfil => `
                <tr>
                    ...
                    <td>
                        <button class="btn-icon edit-perfil" data-id="${perfil.id}" data-nombre="${perfil.nombre}">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-icon danger delete-perfil" data-id="${perfil.id}" data-nombre="${perfil.nombre}">
            ...
            perfilesTableBody.innerHTML = html;
        }
        ...
        function setupSearch() {
            searchInput.addEventListener('input', function(e) {
                ...
                const filteredPerfiles = perfiles.filter(perfil => 
                    ...
                    perfil.id.toString().includes(searchTerm)
                );
        ...
        // ===== CRUD OPERATIONS =====
        function createPerfil(perfilData) {
            $.ajax({
                url: '/api/perfiles',
                method: 'POST',
                data: perfilData,
                success: function(response) {
                    if (response.success) {
                        showAlert('success', 'Perfil creado correctamente');
                        closeModal('perfilModal');
                        loadPerfiles();
                    } else {
                        showAlert('error', 'Error al crear el perfil: ' + response.message);
                    }
                },
                error: function(xhr) {
        ...
        function updatePerfil(id, perfilData) {
            $.ajax({
                url: `/api/perfiles/${id}`,
                method: 'PUT',
                data: perfilData,
                success: function(response) {
                    if (response.success) {
                        ...
                        showAlert('error', 'Error al actualizar el perfil: ' + response.message);
                    }
                },
                error: function(xhr) {
        ...
        function deletePerfil(id) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        showAlert('success', 'Perfil eliminado correctamente');
                        closeModal('confirmModal');
                        loadPerfiles();
                    } else {
                        showAlert('error', 'Error al eliminar el perfil: ' + response.message);
                    }
                },
                error: function(xhr) {
        ...
        function openDeleteModal(id, nombre) {
            document.getElementById('perfilToDelete').textContent = nombre;
            document.getElementById('confirmMessage').textContent = 
                `¿Estás seguro de que quieres eliminar el perfil "${nombre}"?`;
            currentPerfilId = id;
        ...
        function handleFormSubmit() {
            ...
            // Validación básica
            if (!nombre) {
                showFieldError('nombre', 'El nombre del perfil es obligatorio');
                return;
            }

            const perfilData = { nombre: nombre };
        ...
        function setupEventListeners() {
            ...
            document.getElementById('addPerfil').addEventListener('click', openCreateModal);
            document.getElementById('addFirstPerfil').addEventListener('click', openCreateModal);

            // Botón refrescar
            document.getElementById('refreshPerfiles').addEventListener('click', loadPerfiles);

            // Guardar perfil
            document.getElementById('savePerfil').addEventListener('click', handleFormSubmit);

            // Confirmar eliminación
            document.getElementById('confirmDelete').addEventListener('click', function() {
                if (currentPerfilId) {
                    deletePerfil(currentPerfilId);
                }
            });
            ...
            document.getElementById('perfilesTableBody').addEventListener('click', function(e) {
                const target = e.target.closest('button');
                if (!target) return;

                if (target.classList.contains('edit-perfil')) {
                    const id = target.getAttribute('data-id');
                    const nombre = target.getAttribute('data-nombre');
                    openEditModal(id, nombre);
                } else if (target.classList.contains('delete-perfil')) {
                    const id = target.getAttribute('data-id');
                    const nombre = target.getAttribute('data-nombre');
        ...
        function showLoading() {
            loadingPerfiles.style.display = 'block';
            perfilesTableBody.innerHTML = '';
            emptyState.style.display = 'none';
        }

        function hideLoading() {
            loadingPerfiles.style.display = 'none';
        }
